...
class PushNotificationService {
  ...
  Future<void> _showLocalNotification(RemoteMessage message) async {
    ...
    // Determinar canal según data o prioridad
    final bool isCritical = message.data['priority'] == 'high' || 
  ...
  Future<void> registerTokenWithBackend() async {
    ...
    try {
      final accessToken = await _storage.read(key: 'pino.access_token');
      if (accessToken == null) {
      ...
      final url = '${baseUrl.replaceAll(RegExp(r'/$'), '')}/notifications/device-token';
  ...
  Future<void> unregisterTokenWithBackend() async {
    ...
    try {
      ...
      final baseUrl = AppConfig.apiBaseUrl;
      final url =
          '${baseUrl.replaceAll(RegExp(r'/$'), '')}/notifications/unregister-token';

      await dio.post(
        url,
        data: {'token': _token},
      );
      log('✅ Token FCM eliminado de backend: $url', name: 'FCM');
    } catch (e) {
      log('❌ Error eliminando token FCM de backend: $e', name: 'FCM');
    }
  }
